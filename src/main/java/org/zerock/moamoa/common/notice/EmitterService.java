...
public class EmitterService {
    ...
    public SseEmitter subscribe(String username, String lastEventId) {
        String memberCode = userRepository.findByEmailOrThrow(username).getCode();

        // 누가 사용하고 있는 SSE emitter인지 구분
        // 시간을 emitterId에 붙여두면 데이터가 유실된 시점을 알 수 있음
        String emitterId = makeTimeIncludeId(memberCode);
    ...
    private String makeTimeIncludeId(String memberCode) {
        return memberCode + "_" + System.currentTimeMillis();
    }
    ...
    private void sendLostData(String lastEventId, String userCode, String emitterId, SseEmitter emitter) {
        Map<String, Object> eventCaches = emitterRepository.findAllEventCacheStartWithByMemberId(userCode);
